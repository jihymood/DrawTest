...
public class DrawPolygonView2 extends View {
    private int horizontal = 0; //水平方向
    private int vertical = 1;   //垂直方向
    private int isnothororver = 7; //不是水平也不是垂直
    ...
    private List<Point> movePoints = new ArrayList<>();//用于存放变化的点
    private List<Point> linePoints = new ArrayList<>();
    private Paint mPaint = new Paint();//画笔
    private Path mPath = new Path();//路径
    private float x;//上一个拐点x
    private float y;//上一个拐点y
    //当前点
    private float newX;
    private float newY;
    private Point startPoint; // TODO: 2017/7/26
    private float startX;//开始点
    private float startY;//开始点
    private Point lastPoint; // TODO: 2017/7/26
    ...
    private List<Integer> orientationList;//判断手指移动方向的集合
    private boolean drawAble = true;//判断是否可画图
    private boolean isRemoveStartPoint;//是否移除开始点
    private boolean isDrawLine;//是否划线
    ...
    private void actionMove(MotionEvent e) {
        ...
        if (Math.abs(lastX - startX) > 2 || Math.abs(lastY - startY) > 2) {
            ...
            if (Math.abs(lastX - startX) > Math.abs(lastY - startY)) {
                ...
                if (orientation == vertical && !orientationList.contains(horizontal)) {
                    //保存拐点
                ...
                orientationList.add(0, orientation);
                ...
                if (orientation == horizontal && !orientationList.contains(vertical)) {
                    points.add(new Point(newX, newY));
                    //赋值上一个拐点
                    x = newX;
                    y = newY;
...
//        if (!orientationList.contains(vertical)) {//水平 // TODO: 2017/7/25
        if (!orientationList.subList(1, 5).contains(vertical)) {//水平
            //形成多边形集合
...
//        } else if (!orientationList.contains(horizontal)) {//垂直
        } else if (!orientationList.subList(1, 5).contains(horizontal)) {//垂直
            //形成多边形集合
    ...
    public void drawLine(List<Point> points) {
        ...
        linePoints.clear();
        for (int i = downPosition + 1; i < points.size(); i++) {
            linePoints.add(points.get(i));
        }
        for (int i = 0; i < downPosition + 1; i++) {
            linePoints.add(points.get(i));
        }
